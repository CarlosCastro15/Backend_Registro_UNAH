...
//   const deleteMatriculaQuery = `DELETE FROM matricula WHERE id_seccion = ${id_seccion}`;
//   const deleteEvaluarDocenteQuery = `DELETE FROM evaluardocente WHERE id_seccion = ${id_seccion}`;
//   const deleteSeccionQuery = `DELETE FROM seccion WHERE id_seccion = ${id_seccion}`;

//   let num_empleado = null; // Variable para guardar el num_empleado
//   let unidades_valo = null; // Variable para guardar unidades_valo
...
//               // Consulta adicional para obtener num_empleado
//               const numEmpleadoQuery = `SELECT num_empleado FROM seccion WHERE id_seccion = ${id_seccion}`;
//               db.query(numEmpleadoQuery, (error, numEmpleadoResults) => {
//                 if (error) {
//                   console.error('Error al obtener el num_empleado:', error);
//                 } else {
//                   num_empleado = numEmpleadoResults[0].num_empleado;

//                   // Consulta adicional para obtener unidades_valo
//                   const unidadesValoQuery = `
//                                       SELECT c.unidades_valo
//                                       FROM clase c
//                                       JOIN seccion s ON c.id_clase = s.id_clase
//                                       WHERE s.id_seccion = ${id_seccion}`;
//                   db.query(unidadesValoQuery, (error, unidadesValoResults) => {
//                     if (error) {
//                       console.error('Error al obtener unidades_valo:', error);
//                     } else {
//                       unidades_valo = unidadesValoResults[0].unidades_valo;

//                       // Ejecutar el query UPDATE
//                       const updateDocenteQuery = `
//                                               UPDATE docente
//                                               SET unidades_valo = unidades_valo + ${unidades_valo}
//                                               WHERE num_empleado = ${num_empleado}`;
//                       db.query(updateDocenteQuery, (error, updateResults) => {
//                         if (error) {
//                           console.error('Error al actualizar unidades_valo en docente:', error);
//                         } else {
//                           console.log('Unidades Valo actualizado en docente:', updateResults);
//                         }
//                       });
//                     }
//                   });
...
export const eliminarSeccion = (req, res) => {
  ...
  db.query(numEmpleadoQuery, (error, numEmpleadoResults) => {
    if (error) {
      console.error('Error al obtener el num_empleado:', error);
      res.status(500).json({ error: 'Error al obtener el num_empleado' });
    } else {
      if (numEmpleadoResults && numEmpleadoResults.length > 0) {
        num_empleado = numEmpleadoResults[0].num_empleado;
        ...
        db.query(unidadesValoQuery, (error, unidadesValoResults) => {
          if (error) {
            console.error('Error al obtener unidades_valo:', error);
            res.status(500).json({ error: 'Error al obtener unidades_valo' });
          } else {
            if (unidadesValoResults && unidadesValoResults.length > 0) {
              unidades_valo = unidadesValoResults[0].unidades_valo;
              ...
              db.query(updateDocenteQuery, (error, updateResults) => {
                if (error) {
                  console.error('Error al actualizar unidades_valo en docente:', error);
                  res.status(500).json({ error: 'Error al actualizar unidades_valo en docente' });
                } else {
                  console.log('Unidades Valo actualizado en docente:', updateResults);
              ...
              console.error('No se encontraron resultados para unidades_valo');
              res.status(500).json({ error: 'No se encontraron resultados para unidades_valo' });
            }
          }
        });
      } else {
        console.error('No se encontraron resultados para num_empleado');
        res.status(500).json({ error: 'No se encontraron resultados para num_empleado' });
      }
    }
  });
};
