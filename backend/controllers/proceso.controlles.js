import { db } from '../db.js'
 ...
 export const procespDisponibilidad = (req, res) => {
  ...
  export const actualizarDisponibilidadProcesoCarga = (req, res) => {
    const id = req.params.id;
    const disponibilidad = req.body.disponibilidad;
  
    // Consulta SQL para actualizar la disponibilidad
    const sql = `UPDATE proceso_carga
                 SET disponibilidad = ?
                 WHERE id = ?`;
  ...
  export const procesoCargaDisponibilidad = (req, res) => {
    db.query('SELECT * FROM proceso_carga WHERE disponibilidad = TRUE', (error, results) => {
      if (error) {
        console.error('Error al ejecutar la consulta:', error);
        res.status(500).json({ error: 'Error al ejecutar la consulta' });
      } else {
